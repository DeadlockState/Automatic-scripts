...
if [ "$USER" = "root" ] ; then
	...
	echo ""
	echo " 7. Configuring CouchPotato..."
	echo ""
	...
	replace "CP_USER=couchpotato" "CP_USER=root" -- /etc/default/couchpotato
	
	replace "CP_HOME=" "CP_HOME=/opt/CouchPotatoServer" -- /etc/default/couchpotato
	...
	if [ "$INSTALL_TORRENT" = "r" ] ; then
		echo " 8. Installing rTorrent + ruTorrent..."
		echo ""

		# 
	else
		echo " 8. Installing Transmission..."
		echo ""
		
		apt-get install -y transmission-daemon
		
		echo ""
		echo " 9. Configuring Transmission..."
		echo ""
		
		sed -i 's/(rpc-password\": {(.+)})/rpc-password\": transmission/g' /etc/transmission-daemon/settings.json
		
		replace "rpc-whitelist-enabled\": true," "rpc-whitelist-enabled\": false," -- /etc/transmission-daemon/settings.json
	...
	echo "SickRage : http://"$IP_ADDRESS":8081"
